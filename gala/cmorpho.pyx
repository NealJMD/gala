...
def smallest_int_dtype(number, is_signed=False, mindtype=None):
    if number < 0: is_signed = True
    if not is_signed:
...
def flood_fill(im, start, acceptable, raveled = False):
    ...
    if im.ndim == 3:
        ...
        s = np.array(start)
        matches = flood_fill_3d(im, s, a)
        if raveled:
            formatted = matches.T
            return np.ravel_multi_index(formatted, im.shape)
        else:
            return [tuple(row) for row in matches]
    else: return flood_fill_nd(im, start, acceptable, raveled)
        

def flood_fill_nd(im, start, acceptable, raveled=False):
    ...
    while frontier:
        ...
        for base_point in frontier:
            adjacent = adjacent_points(base_point, im.shape)
            for p in adjacent:
                point = tuple(p)
                if point in matches or point == base_point: continue
...
cdef np.ndarray[np.int_t, ndim=2] flood_fill_3d(np.ndarray[np.int_t, ndim=3] im, 
            np.ndarray[np.int_t, ndim=1] start, np.ndarray[np.int_t, ndim=1] acceptable):
    ...
    cdef int in_matches, in_acceptable, base_point_ii, p_ii, new_frontier_size
    ...
    for ii in range(ndim): shape[ii] = im.shape[ii]
    frontier[0] = start
    matches[0] = start
    ...
    while frontier_size > 0:
        ...
        for base_point_ii in range(frontier_size):
            ...
            adjacent = c_adjacent_points(base_point, shape)
            for p_ii in range(adjacent.shape[0]):
                p = adjacent[p_ii]
                in_matches = any(np.equal(matches, p).all(1))
                if in_matches: continue
                in_acceptable = any(np.equal(acceptable, im[p[0],p[1],p[2]]))
                if not in_acceptable: continue
                new_frontier[new_frontier_size] = p
                matches[matches_size] = p
                matches_size += 1
                new_frontier_size += 1
                if matches_size >= matches.shape[0]:
                    matches = expand_2darray(matches)
                if new_frontier_size >= new_frontier.shape[0]:
                    new_frontier = expand_2darray(new_frontier)
        frontier = new_frontier
        frontier_size = new_frontier_size
    return matches[0:matches_size, :]
 
    
cdef np.ndarray[np.int_t, ndim=2] expand_2darray(np.ndarray[np.int_t, ndim=2] a):
    cdef int rows = a.shape[0]
    cdef int cols = a.shape[1]
    cdef np.ndarray[np.int_t, ndim=2] expanded = np.empty([rows * 2, cols], dtype=np.int)
    expanded[0:rows, :] = a
    return expanded

cdef np.ndarray[np.int_t, ndim=2] c_adjacent_points(np.ndarray[np.int_t, ndim=1] point, 
                                        np.ndarray[np.int_t, ndim=1] shape):
    cdef int dimensions = point.size
